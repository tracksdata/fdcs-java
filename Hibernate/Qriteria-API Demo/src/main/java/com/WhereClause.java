...
public class WhereClause {
	...
	public static void main(String[] args) {
		...
		//criteriaQuery.select(root).where(criteriaBuilder.equal(root.get("empId"), 10));
		criteriaQuery.select(root).where(criteriaBuilder.greaterThanOrEqualTo(root.get("salary"), 3463743));
